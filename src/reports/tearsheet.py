...
- Latest-year Cash Flow waterfall chart
...
def create_roe_roce_chart(data, ticker):
    ...
    Historical ROE is available from financial_ratios.

    Historical ROCE is not available in the database.
    Therefore, the latest company-level ROCE is displayed
    as a reference line across the historical ROE period.
...
def create_cash_flow_chart(df, ticker):
    """
    Create a latest-year cash flow waterfall chart.

    The waterfall starts from zero and sequentially applies:
    CFO -> CFI -> CFF.

    A final Net Cash Flow bar shows the resulting total.

    No connector lines are used.
    No dotted or dashed lines are used.
    """
    ...
    final_x = len(values) + 1

    if not pd.isna(net_cash):
    ...
    ax.set_title(
        f"Cash Flow Waterfall - Latest Year ({int(row['year'])})",
        fontsize=10,
        fontweight="bold",
        loc="left",
    )
