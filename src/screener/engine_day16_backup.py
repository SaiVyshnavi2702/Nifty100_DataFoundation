...
def apply_filters(df, filters):
    ...
    # 1. ROE minimum
    result = apply_min_filter(result, "return_on_equity_pct", filters.get("roe_min"))
    ...
    if de_max is not None:
        ...
        result = result[
            financials
            | (result["debt_to_equity"].notna() & (result["debt_to_equity"] <= de_max))
        ]

    # 3. Free Cash Flow minimum
    result = apply_min_filter(
        result, "free_cash_flow_cr", filters.get("free_cash_flow_min")
    )

    # 4. Revenue CAGR 5yr minimum
    result = apply_min_filter(
        result, "revenue_cagr_5yr", filters.get("revenue_cagr_5yr_min")
    )

    # 5. PAT CAGR 5yr minimum
    result = apply_min_filter(result, "pat_cagr_5yr", filters.get("pat_cagr_5yr_min"))
    ...
    # 7. P/E maximum
    result = apply_max_filter(result, "pe_ratio", filters.get("pe_ratio_max"))

    # 8. P/B maximum
    result = apply_max_filter(result, "pb_ratio", filters.get("pb_ratio_max"))

    # 9. Dividend Yield minimum
    result = apply_min_filter(
        result, "dividend_yield_pct", filters.get("dividend_yield_min")
    )
    ...
    result = apply_min_filter(result, "market_cap_crore", filters.get("market_cap_min"))

    # 12. Net Profit minimum
    result = apply_min_filter(result, "net_profit", filters.get("net_profit_min"))

    # 13. EPS CAGR minimum
    result = apply_min_filter(result, "eps_cagr_5yr", filters.get("eps_cagr_min"))

    # 14. Asset Turnover minimum
    result = apply_min_filter(
        result, "asset_turnover", filters.get("asset_turnover_min")
    )

    # 15. Sales minimum
    result = apply_min_filter(result, "sales", filters.get("sales_min"))
...
if __name__ == "__main__":
    ...
    if not result.empty:
        print()
        print(
            result[["company_id", "composite_quality_score"]]
            .head(10)
