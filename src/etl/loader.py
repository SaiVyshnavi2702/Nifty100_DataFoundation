...
FILES = {
    ...
    "financial_ratios": os.path.join(SUPPORTING_PATH, "financial_ratios.xlsx"),
    "market_cap": os.path.join(SUPPORTING_PATH, "market_cap.xlsx"),
    "peer_groups": os.path.join(SUPPORTING_PATH, "peer_groups.xlsx"),
    "sectors": os.path.join(SUPPORTING_PATH, "sectors.xlsx"),
    "stock_prices": os.path.join(SUPPORTING_PATH, "stock_prices.xlsx"),
}
...
def normalize_common_columns(df):
    ...
    if "year" in df.columns:
        ...
        df["period"] = df["year"].apply(normalize_period)
...
def load_excel_file(path):
    ...
    if absolute_path.startswith(absolute_core_path + os.sep):
    ...
    print("Reading Excel with header row:", header_row)
...
def validate_dataset_columns(table_name, df, connection):
    """Validate dataset columns."""
    database_columns = connection.execute(f"PRAGMA table_info({table_name})").fetchall()

    if not database_columns:

        raise RuntimeError(f"Database table does not exist: {table_name}")

    database_column_names = {row[1] for row in database_columns}

    dataframe_columns = set(df.columns)

    missing_in_database = dataframe_columns - database_column_names

    if missing_in_database:

        raise RuntimeError(
            f"Table '{table_name}' is missing these columns: {sorted(missing_in_database)}"
        )


def check_duplicate_keys(table_name, df):
    ...
    duplicate_count = df.duplicated(subset=keys, keep=False).sum()
    ...
    duplicate_rows = df[df.duplicated(subset=keys, keep=False)]
    ...
    print(
        duplicate_rows[list(dict.fromkeys(display_columns))]
        .sort_values(keys)
        .to_string(index=False)
    )

    raise RuntimeError(f"Duplicate logical keys found in {table_name}")
...
def load_datasets_to_database(connection, datasets):
    ...
    for table_name in LOAD_ORDER:
        ...
        source_file = FILES[table_name]

        try:

            validate_dataset_columns(table_name, df, connection)

            check_duplicate_keys(table_name, df)

            rows_to_load = len(df)

            print(f"Loading {rows_to_load} rows into '{table_name}'...")
            ...
            print("Rows currently in database:", database_count)
...
def write_load_audit(audit_records):
    """Write load audit."""
    audit_directory = os.path.dirname(AUDIT_PATH)
...
def verify_tables(connection):
    ...
    actual_tables = {row[0] for row in actual_tables}
    ...
    print("Tables created:", len(actual_tables))
...
def verify_foreign_keys(connection):
    ...
    if violations:
        ...
        raise RuntimeError("Foreign key violations detected.")
...
def main():
    ...
    for name, df in datasets.items():
        ...
        print(f"{name:<20} rows={len(df):<6} columns={len(df.columns)}")
    ...
    try:
        ...
        audit_records = load_datasets_to_database(connection, datasets)

        connection.commit()

        verify_database(connection)

        write_load_audit(audit_records)

        print("\nSQLite database loading completed successfully.")

        print("Day 05 full data load completed.")
