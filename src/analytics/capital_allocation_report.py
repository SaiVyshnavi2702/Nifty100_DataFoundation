...
def verify_capital_allocation(df):
    ...
    if not columns_valid:
        raise ValueError(
            "Capital allocation columns are not correct."
        )

    if company_count == 0:
        raise ValueError(
            "No companies found in capital_allocation.csv."
        )

    if df["pattern_label"].isna().any():
        raise ValueError(
            "Some capital allocation rows have missing pattern labels."
        )

    invalid_patterns = sorted(
        set(df["pattern_label"])
    ...
    if invalid_patterns:
        raise ValueError(
            f"Unexpected capital allocation patterns: {invalid_patterns}"
        )
    ...
    min_year_count = int(
        company_year_counts.min()
    )

    max_year_count = int(
        company_year_counts.max()
    )
...
def generate_latest_year_distribution(df):
    ...
    print()
    print(
        f"CAPITAL ALLOCATION DISTRIBUTION - LATEST YEAR ({latest_year})"
    )
...
def update_cashflow_intelligence(df):
    ...
    intelligence_df["company_id"] = (
        intelligence_df["company_id"]
        .astype(str)
    )

    latest_allocation["company_id"] = (
        latest_allocation["company_id"]
        .astype(str)
    )
    ...
    print(
        "Added column: capital_allocation"
    )
...
def verify_day32_outputs(
    ...
    latest_company_count = capital_df[
    ...
    pattern_change_columns_valid = (
        list(pattern_changes_df.columns)
        == [
            "company_id",
            "from_year",
            "to_year",
    ...
    if not pattern_changes_df.empty:
        pattern_changes_valid = (
            (
                pattern_changes_df["previous_pattern"]
                != pattern_changes_df["new_pattern"]
            ).all()
        )

    outputs_exist = (
        os.path.exists(
            CAPITAL_ALLOCATION_INPUT
        )
        and os.path.exists(
            CASHFLOW_INTELLIGENCE_INPUT
        )
    ...
    print(
        "Pattern changes columns valid:",
        pattern_change_columns_valid
    )
    print(
        "Pattern changes valid:",
        pattern_changes_valid
    )
    print(
...
def main():
    ...
    print()
    print(
        "Loading capital allocation data..."
    )

    capital_df = load_capital_allocation()

    company_count, years = verify_capital_allocation(
        capital_df
    )

    print()
    print(
        "Generating latest-year distribution..."
    )

    distribution = generate_latest_year_distribution(
        capital_df
    )

    print()
    print(
        "Updating cashflow intelligence Excel..."
    )
    ...
    print()
    print(
        "Detecting year-over-year pattern changes..."
    )

    pattern_changes_df = generate_pattern_changes(
        capital_df
    )
    ...
    if pattern_changes_df.empty:
        ...
        print(
            pattern_changes_df.head(20).to_string(
                index=False
            )
        )

    verify_day32_outputs(
        capital_df,
        intelligence_df,
        pattern_changes_df
    )
